...
const Header = () => {
  const [dropdownShown, toggleDropdown] = useToggle(false)
  const [burgerShown, toggleBurger] = useToggle(false)
  
  return (
    <header className='bg-[#17161A]/50 fixed top-0 left-0 w-full z-10'>
      <Container>
        <nav className="flex items-center justify-between text-white font-semibold uppercase font-drone text-sm px-6 py-2 lg:px-12 lg:py-6">
          <ul className="hidden lg:flex items-center justify-around">
            ...
            <li className="mr-6">
              <a href="#" className="text-lg">Our teams</a>
            </li>
            <li className="mr-6">
              <a href="#" className="text-lg">Marketplace roadmap</a>
            </li>
            <li className="mr-6">
          ...
          <div className="lg:hidden flex items-center">
            <button className="bg-gradient-to-r from-[#DA458F] to-[#DA34DD] hover:from-[#DA458F] hover:to-[#DA34DD] text-white font-bold py-1 px-3 rounded mr-4 z-20" onClick={toggleBurger}>
              <FontAwesomeIcon icon={faCaretDown} className={`transition-transform duration-300 ${dropdownShown ? 'rotate-180' : 'rotate-0'}`} />
            </button>
            <div className={`fixed top-0 left-0 h-full w-full z-10 bg-[#17161A]/75 ${burgerShown ? 'opacity-100 pointer-events-auto translate-y-2' : 'opacity-0 pointer-events-none -translate-y-8'}`}>
          ...
          <div className="flex items-center">
            ...
            <div className="relative">
              ...
              <div className={`absolute right-0 mt-2 py-2 w-48 bg-white rounded-md shadow-lg ${dropdownShown ? 'opacity-100 pointer-events-auto translate-y-2' : 'opacity-0 pointer-events-none translate-y-0'} ${dropdownShown ? 'block' : ''} transition-all duration-300`}>
                ...
                <a href="#" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Italian</a>
              </div>
            </div>
          </div>
        </nav>
