...
const CriteriaSelect = ({ title, options }: CriteriaSelectProps) => {
  ...
  return (
    <PropertyFilterItem title={title} titleClasses="text-secondary">
      <div className="relative mt-4 w-full">
        ...
        <FontAwesomeIcon
          icon={faAngleDown}
          className={`text-white absolute right-4 top-1/2 -translate-y-1/2 transition duration-100 ${isFocused ? "rotate-180" : "rotate-0"}`}
        />
      </div>
    </PropertyFilterItem>
  )
